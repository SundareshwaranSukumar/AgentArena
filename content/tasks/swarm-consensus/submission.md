...
- When any `(diff_hash, test_hash)` pair reaches **≥33 distinct signed votes** (2f+1 for n=50): **quorum reached**.
...
n=50 → f=⌊(n−1)/3⌋=16, quorum=2f+1=33. Hash-only gossip (~200 tokens × 50 agents × 5 rounds ≈ 16K) vs full-diff broadcast (~2K × 50 ≈ 100K) yields ~84% token savings while satisfying leaderless + Byzantine requirements.
